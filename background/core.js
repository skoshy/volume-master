...
var a = {
  ...
  createAudio: function (currentTab, tabLevel, stream) {
    ...
    currentTab.streamOutput = currentTab.audioCtx.createMediaStreamSource(
      currentTab.stream
    );
  ...
  deInit: function (id) {
    ...
    Object.prototype.hasOwnProperty.call(tabsGaines, id) &&
      tabsGaines[id].audioCtx.close().then(function () {
        delete tabsGaines[id];
        delete tabsLevels[id];
      });
  },
  getTab: function (id) {
    if (tabsLevels.hasOwnProperty(id)) return true;
    return false;
  },
  ...
  visuInit: function (id, port) {
    if (!id) {
      chrome.tabs.query({ windowType: "normal", audible: true }, function (
        tabArray
      ) {
        tabArray.forEach((e, n) => {
          if (tabsGaines.hasOwnProperty(e.id)) id = e.id;
        });
        a.visuInit(id, port);
      });
      return;
