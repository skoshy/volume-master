...
chrome.tabs.onRemoved.addListener(function (a) {
  Object.prototype.hasOwnProperty.call(tabsGaines, a) &&
    tabsGaines[a].audioCtx.close().then(function () {
      delete tabsLevels[a];
      delete tabsGaines[a];
    });
});
...
chrome.commands.onCommand.addListener(function (command) {
  ...
  if (!hotkeysType[parseInt(command[command.length - 1]) - 1]) return;
...
chrome.tabCapture.onStatusChanged.addListener(function (info) {
  ...
  if (sysOS == "mac" && info.fullscreen) {
    ...
    if (!prevFullScreen) {
      chrome.windows.getCurrent(function (win) {
        prevWindow = win;
        if (fscreen) chrome.windows.update(win.id, { state: "fullscreen" });
      });
    }
  } else if (sysOS != "mac" && prevWindow) {
    chrome.windows.getCurrent(function (win) {
      if (fscreen) chrome.windows.update(win.id, { state: prevWindow.state });
    });
  } else if (sysOS == "mac" && prevWindow) {
    chrome.tabs.move(
      info.tabId,
      { windowId: prevWindow.id, index: prevWindow.tabIndex },
      () => {
...
chrome.extension.onConnect.addListener(function (port) {
  ...
  // if (true) {}
  // currentWindow: true
  chrome.tabs.query({ windowType: "normal", active: true }, function (
    tabArray
  ) {
    if (tabArray[0].audible && !a.getTab(tabArray[0].id)) {
      ...
      port.postMessage({ tabsLevels: tabsLevels, curTab: tabArray[0] });
      a.visuInit(tabArray[0].audible ? tabArray[0].id : false, port);
      if (a.getTab(tabArray[0].id))
        port.postMessage({
          type: "equalizerSettings",
          data: tabsGaines[tabArray[0].id].equalizer,
        });
    }
  });
});
