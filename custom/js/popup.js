  ...
  .controller("AppCtrl", function ($scope, $mdDialog) {
    ...
    $scope.showAlert = function (ev) {
      if (!false) {
        ...
        chrome.runtime.sendMessage({
          how: "promotion",
          what: "Interested in the PRO equalizer",
        });
      } else {
        $scope.showEqualizer = !$scope.showEqualizer;
        chrome.runtime.sendMessage({
          how: "popup",
          what: "Equalizer open/close",
        });
      }
    };
    ...
    $scope.promote1Show = function (e) {
      if (e) {
        ...
        chrome.runtime.sendMessage({
          how: "promotion",
          what: "Interested in the promote 1",
        });
      } else {
        $scope.promote1 = !1;
        chrome.runtime.sendMessage({
          how: "promotion",
          what: "Close the promote 1",
        });
      }
    };
    $scope.promote1Redirect = function () {
      chrome.runtime.sendMessage({
        how: "promotion",
        what: "Redirected to the promote 1",
      });
      setTimeout(function () {
        chrome.tabs.create({
          url: "https://www.verblike.com/instafly/chrome-web-store-link/",
          active: !0,
          selected: !0,
        });
      }, 10);
    ...
    port.onMessage.addListener(function (msg) {
      ...
      $scope.changing.currentLevel = function () {
        ...
        $scope.controlledTabs.find(function (el) {
          el.id == msg.curTab.id
            ? (el.volumeLevel = $scope.currentLevel)
            : false;
        });
      };
      $scope.changing.controlledTabs = function (id, val) {
        changeVolume(id, val);
        if (id == msg.curTab.id) $scope.currentLevel = val;
      };
      $scope.changing.equalizer = function () {
        port.postMessage({
          id: parseInt(msg.curTab.id),
          val: angular.copy($scope.equalizer),
          type: "equalizer",
        });
      };
      $scope.redirect = function (id) {
        if (id == "options") {
          ...
          chrome.tabs.create({
            url:
              "https://chrome.google.com/webstore/detail/volume-manager/abnhonfioiokelhdappjknfaannlncac/reviews",
            active: !0,
            selected: !0,
          });
        } else {
      ...
      chrome.tabs.query({ audible: !0 }, function (tabs) {
        ...
        chrome.runtime.sendMessage({
          ...
          what:
            "Tabs count: " +
            ($scope.noizeTabs.length + $scope.controlledTabs.length),
        });
